const PROBLEMATIC_IDENTIFIERS = {
  ...
  MutationObserver: 'Use `getMutationObserverConstructor` from @renanvizza-prontmed/browser-rum-core instead',

  // Using the patched `setTimeout` from Zone.js triggers a rendering loop in some Angular
  // component, see issue PR #2030
  setTimeout: 'Use `setTimeout` from @renanvizza-prontmed/browser-core instead',
  clearTimeout: 'Use `clearTimeout` from @renanvizza-prontmed/browser-core instead',
  ...
  setInterval: 'Use `setInterval` from @renanvizza-prontmed/browser-core instead',
  clearInterval: 'Use `clearInterval` from @renanvizza-prontmed/browser-core instead',

  // Using the patched `addEventListener` from Zone.js might trigger a memory leak in Firefox, see
  // PR #1860
  addEventListener: 'Use `addEventListener` from @renanvizza-prontmed/browser-core instead',
  removeEventListener: 'Use `addEventListener().stop` from @renanvizza-prontmed/browser-core instead',
}
